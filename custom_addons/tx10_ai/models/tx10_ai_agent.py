...
class Tx10AiAgent(models.AbstractModel):
    _name = "tx10.ai.agent"
    _description = "TX10 AI — Tool Registry and Dispatcher"
        ...
        return [
            ...
            },
            {
                ...
                "function": {
                    ...
                    "description": "Render a link to a specific record form view. Returns an HTML link.",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "model": {"type": "string"},
            ...
            },
            {
                ...
                "function": {
                    ...
                    "description": "Render a link to the list view for a model. Returns an HTML link.",
                    "parameters": {
                        "type": "object",
    ...
    def safe_execute_tool(self, tool_name, args, tool_call_id=""):
        ...
        try:
            ...
            _logger.warning(
                "tx10_ai agent: tool %r validation error: %s",
                tool_name,
                exc,
            )
            return {
            ...
            _logger.warning("tx10_ai agent: tool %r access denied: %s", tool_name, exc)
            return {
                "ok": False,
                "error": "access_denied",
                "tool_call_id_placeholder": tool_call_id,
    ...
    # ------------------------------------------------------------------
    # Tool implementations (NAVIGATE — server-side HTML links)
    # ------------------------------------------------------------------
    ...
    def _tool_open_model_list(self, args):
        ...
        link = Markup("<a href='/odoo/action-%s'>%s</a>") % (
            action.id,
            self.env[model]._description or model,
        )
        return {"html_link": link, "status": "link_rendered"}
    ...
    def _tool_create_record(self, args, chat_id):
        ...
        chat = self.env["tx10.ai.chat"].browse(int(chat_id))
